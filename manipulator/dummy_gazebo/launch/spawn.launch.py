...
def launch_action_robot_spawner(dh_parameters, robot_description, position, robot_name=""):
    ...
    parameters.append({'robot_description':robot_desc_xml, "frame_prefix": robot_name + "/"})
    ...
    # Create Gazebo Robot Spawner
    robot_spawner = Node(
        package="gazebo_ros",
        executable="spawn_entity.py",
        output="screen",
        arguments=[
            "-topic", "{}/robot_description".format(robot_name),
            "-entity", str(robot_name),
            '-x', str(position[0]) ,
            '-y', str(position[1]) ,
            '-z', str(position[2]) ,
            '-R', '0.0',
