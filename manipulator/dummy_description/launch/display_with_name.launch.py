...
# [TODO 1: Add robot_name arguments to have rviz disp for each robot]
def launch_action_rviz(rviz_config, robot_name=""):
    ...
    if robot_name:
        with open(rviz_config.path, "r") as rviz_file:
            rviz_param = yaml.load(rviz_file, yaml.SafeLoader)
            # Edit rviz_param so that the namespace and prefix is set
        ...
        # No namespace
        rviz_config_path = rviz_config.path
    #####################################
    rviz = Node(
        package='rviz2',
        executable='rviz2',
...
def launch_action_robot(dh_parameters, robot_description, robot_name=""):
    ...
    # Code Here
    #####################################
    parameters = [{'robot_description':robot_desc_xml, 'frame_prefix': robot_name + "/"}]
...
def generate_launch_description():
    ...
    robot_name = "doomy_1"
    #####################################
    rviz = launch_action_rviz(rviz_config, robot_name=robot_name)
    actions = launch_action_robot(dh_parameters, robot_description, robot_name=robot_name)
